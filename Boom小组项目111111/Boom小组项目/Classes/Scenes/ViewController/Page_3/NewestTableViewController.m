...
#import "NewestTableViewController.h"

@interface NewestTableViewController () <UITableViewDataSource,UITableViewDelegate>
...
- (void)pullToLoadData
{
    ...
    self.tableView.footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            NSString * url_string = [NSString stringWithFormat:@"http://www.molyo.com//mExperience/getList?cityId=%@&shopId=&pageSize=8&currentPage=%ld&accessToken=&netWork=wifi", [[NSUserDefaults standardUserDefaults] valueForKey:@"cityId"], ++self.currentPage];
...
- (void)dropToRefresh
{
    ...
    self.tableView.header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            // 数据解析
            [self requestDataWithListString:[NSString stringWithFormat:@"http://www.molyo.com//mExperience/getList?cityId=%@&shopId=&pageSize=8&currentPage=1&accessToken=&netWork=wifi", [[NSUserDefaults standardUserDefaults] valueForKey:@"cityId"]]];
...
{
    if (!_NewestTableView) {
        self.NewestTableView = [UITableView new];
    }
